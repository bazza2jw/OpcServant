...
DYNAMIC_SERIALIZATION_INFO (UnsignedValue, 1, "{2d9ca199-9472-403a-befa-95c54bb9d1f8}");
...
bool NumberValue::ToBool (const ValueConstPtr& val)
{
    return Value::Cast<NumberValue> (val)->ToBool ();
}

bool NumberValue::ToBool (const ValuePtr& val)
{
    return Value::Cast<NumberValue> (val)->ToBool ();
}

bool NumberValue::ToBool (Value* val)
{
    return Value::Cast<NumberValue> (val)->ToBool ();
}

unsigned NumberValue::ToUnsigned (const ValueConstPtr& val)
{
    return Value::Cast<NumberValue> (val)->ToUnsigned ();
}

unsigned NumberValue::ToUnsigned(const ValuePtr& val)
{
    return Value::Cast<NumberValue>(val)->ToUnsigned ();
}

unsigned NumberValue::ToUnsigned (Value* val)
{
    return Value::Cast<NumberValue> (val)->ToUnsigned ();
}
