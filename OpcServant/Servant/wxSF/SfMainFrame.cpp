...
#include "SfMainFrame.h"
#include "Art.h"
#include "Ids.h"

BEGIN_EVENT_TABLE(SfMainFrm, wxFrame)
    EVT_CLOSE(SfMainFrm::OnClose)
    EVT_MENU(wxID_EXIT, SfMainFrm::OnExit)
    EVT_MENU(wxID_NEW, SfMainFrm::OnNew)
    EVT_MENU(wxID_OPEN, SfMainFrm::OnLoad)
    EVT_MENU(wxID_SAVE, SfMainFrm::OnSave)
    EVT_MENU(wxID_UNDO, SfMainFrm::OnUndo)
    EVT_MENU(wxID_REDO, SfMainFrm::OnRedo)
    EVT_MENU(wxID_COPY, SfMainFrm::OnCopy)
    EVT_MENU(wxID_CUT, SfMainFrm::OnCut)
    EVT_MENU(wxID_PASTE, SfMainFrm::OnPaste)
    EVT_MENU(wxID_ABOUT, SfMainFrm::OnAbout)
    EVT_MENU(wxID_SELECTALL, SfMainFrm::OnSelectAll)
    EVT_MENU(IDM_SAVEASBITMAP, SfMainFrm::OnExportToBMP)
    EVT_MENU(wxID_PRINT, SfMainFrm::OnPrint)
    EVT_MENU(wxID_PREVIEW, SfMainFrm::OnPrintPreview)
    EVT_MENU(wxID_PAGE_SETUP, SfMainFrm::OnPageSetup)
    EVT_MENU_RANGE(IDM_AUTOLAYOUT_FIRST, IDM_AUTOLAYOUT_LAST, SfMainFrm::OnAutoLayout)
    EVT_COMMAND_SCROLL(wxID_ZOOM_FIT, SfMainFrm::OnSlider)
    EVT_TOOL_RANGE(IDT_FIRST_TOOLMARKER, IDT_LAST_TOOLMARKER, SfMainFrm::OnTool)
    EVT_COLOURPICKER_CHANGED(IDT_COLORPICKER, SfMainFrm::OnHowerColor)
    EVT_UPDATE_UI(wxID_COPY, SfMainFrm::OnUpdateCopy)
    EVT_UPDATE_UI(wxID_CUT, SfMainFrm::OnUpdateCut)
    EVT_UPDATE_UI(wxID_PASTE, SfMainFrm::OnUpdatePaste)
    EVT_UPDATE_UI(wxID_UNDO, SfMainFrm::OnUpdateUndo)
    EVT_UPDATE_UI(wxID_REDO, SfMainFrm::OnUpdateRedo)
    EVT_UPDATE_UI_RANGE(IDT_FIRST_TOOLMARKER, IDT_LAST_TOOLMARKER, SfMainFrm::OnUpdateTool)
    EVT_UPDATE_UI_RANGE(IDM_AUTOLAYOUT_FIRST, IDM_AUTOLAYOUT_LAST, SfMainFrm::OnUpdateAutoLayout)
    EVT_IDLE(SfMainFrm::OnIdle)
END_EVENT_TABLE()
...
SfMainFrm::SfMainFrm( wxWindow* parent ) : _SfMainFrm( parent )
{
...
void SfMainFrm::OnClose(wxCloseEvent& WXUNUSED(event))
{
	CleanUp();
}
...
void SfMainFrm::OnExit(wxCommandEvent& WXUNUSED(event))
{
    CleanUp();
}

void SfMainFrm::OnNew(wxCommandEvent& WXUNUSED(event))
{
...
void SfMainFrm::OnLoad(wxCommandEvent& WXUNUSED(event))
{
...
void SfMainFrm::OnSave(wxCommandEvent& WXUNUSED(event))
{
...
void SfMainFrm::OnUndo(wxCommandEvent& WXUNUSED(event))
{
	m_pShapeCanvas->Undo();
}

void SfMainFrm::OnUpdateUndo(wxUpdateUIEvent& event)
{
	if( m_pShapeCanvas ) event.Enable(m_pShapeCanvas->CanUndo());
}

void SfMainFrm::OnRedo(wxCommandEvent& WXUNUSED(event))
{
	m_pShapeCanvas->Redo();
}

void SfMainFrm::OnUpdateRedo(wxUpdateUIEvent& event)
{
	if( m_pShapeCanvas ) event.Enable(m_pShapeCanvas->CanRedo());
}

void SfMainFrm::OnCopy(wxCommandEvent& WXUNUSED(event))
{
	m_pShapeCanvas->Copy();
}

void SfMainFrm::OnUpdateCopy(wxUpdateUIEvent& event)
{
	if( m_pShapeCanvas ) event.Enable(m_pShapeCanvas->CanCopy());
}

void SfMainFrm::OnCut(wxCommandEvent& WXUNUSED(event))
{
	m_pShapeCanvas->Cut();
}

void SfMainFrm::OnUpdateCut(wxUpdateUIEvent& event)
{
	if( m_pShapeCanvas ) event.Enable(m_pShapeCanvas->CanCut());
}

void SfMainFrm::OnPaste(wxCommandEvent& WXUNUSED(event))
{
	m_pShapeCanvas->Paste();
}

void SfMainFrm::OnUpdatePaste(wxUpdateUIEvent& event)
{
	if( m_pShapeCanvas ) event.Enable(m_pShapeCanvas->CanPaste());
}

void SfMainFrm::OnSelectAll(wxCommandEvent& WXUNUSED(event))
{
	m_pShapeCanvas->SelectAll();
}

void SfMainFrm::OnAbout(wxCommandEvent& WXUNUSED(event))
{
...
void SfMainFrm::OnExportToBMP(wxCommandEvent& WXUNUSED(event))
{
...
void SfMainFrm::OnTool(wxCommandEvent& event)
{
...
void SfMainFrm::OnUpdateTool(wxUpdateUIEvent& event)
{
    switch(event.GetId())
    {
        case IDT_GRID:
...
void SfMainFrm::OnUpdateAutoLayout(wxUpdateUIEvent& event)
{
...
void SfMainFrm::OnPrint(wxCommandEvent& WXUNUSED(event))
{
    m_pShapeCanvas->Print();
}

void SfMainFrm::OnPrintPreview(wxCommandEvent& WXUNUSED(event))
{
    m_pShapeCanvas->PrintPreview();
}

void SfMainFrm::OnPageSetup(wxCommandEvent& WXUNUSED(event))
{
...
void SfMainFrm::OnSlider(wxScrollEvent& WXUNUSED(event))
{
	m_pShapeCanvas->SetScale(double(m_pZoomSlider->GetValue())/50);
	m_pShapeCanvas->Refresh(false);
}

void SfMainFrm::OnHowerColor(wxColourPickerEvent& event)
{
	m_pShapeCanvas->SetHoverColour(event.GetColour());
}

void SfMainFrm::OnAutoLayout(wxCommandEvent& event)
{
	m_AutoLayout.Layout( m_pShapeCanvas, m_pAutoLayoutMenu->GetLabel( event.GetId() ) );
	m_pShapeCanvas->SaveCanvasState();
}

void SfMainFrm::OnIdle(wxIdleEvent& event)
{
