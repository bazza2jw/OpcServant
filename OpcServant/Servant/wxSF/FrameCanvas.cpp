...
void FrameCanvas::OnLeftDown(wxMouseEvent& event)
{
    ...
    case SfMainFrm::modeBITMAP:
		...
		break;
    case SfMainFrm::modeTEXT:
    case SfMainFrm::modeEDITTEXT:
		{
		    wxTextEntryDialog dlg(this, wxT(""), wxT("Enter text"), wxT("Hello World!"));

		    if(dlg.ShowModal() == wxID_OK)
		    {
                if(m_pParentFrame->GetToolMode() == SfMainFrm::modeTEXT)
    ...
    case SfMainFrm::modeGRID:
    case SfMainFrm::modeFLEXGRID:
		{
            if( m_pParentFrame->GetToolMode() == SfMainFrm::modeGRID )
    ...
    case SfMainFrm::modeROUNDORTHOLINE:
        {
            if(GetMode() == modeREADY)
            {
    ...
    case SfMainFrm::modeSTANDALONELINE:
		{
	...
	if( pShape )
	{
        ...
        {
            m_pParentFrame->SetToolMode(SfMainFrm::modeDESIGN);
        }
