    ...
    okButton (new wxButton (this, wxID_OK, NE::LocalizeString (L"OK")))
{
    ...
    for (size_t paramIndex = 0; paramIndex < paramInterface->GetParameterCount (); ++paramIndex) {
        ...
        } else if (type == NUIE::ParameterType::Unsigned) {
            if (DBGVERIFY (NE::Value::IsType<NE::UnsignedValue> (value))) {
        ...
        else if (type == NUIE::ParameterType::FileName) {
            if (DBGVERIFY (NE::Value::IsType<NE::StringValue> (value))) {
                wxTextCtrl* textControl = new wxTextCtrl (this, controlId, NUIE::ParameterValueToString (value, type)); // change to a file browser
                control = textControl;
            }
