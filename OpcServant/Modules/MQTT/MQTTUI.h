//////////////////////////////////////////////////////////////////////
// This file was auto-generated by codelite's wxCrafter Plugin
// wxCrafter project file: MQTTUI.wxcp
// Do not modify this file by hand!
//////////////////////////////////////////////////////////////////////

#ifndef _MODULES_MQTT_MQTTUI_BASE_CLASSES_H
#define _MODULES_MQTT_MQTTUI_BASE_CLASSES_H

#include <wx/settings.h>
#include <wx/xrc/xmlres.h>
#include <wx/xrc/xh_bmp.h>
#include <wx/dialog.h>
#include <wx/iconbndl.h>
#include <wx/artprov.h>
#include <wx/sizer.h>
#include <wx/stattext.h>
#include <wx/textctrl.h>
#include <wx/spinctrl.h>
#include <wx/checkbox.h>
#include <wx/button.h>
#if wxVERSION_NUMBER >= 2900
#include <wx/persist.h>
#include <wx/persist/toplevel.h>
#include <wx/persist/bookctrl.h>
#include <wx/persist/treebook.h>
#endif

#ifdef WXC_FROM_DIP
#undef WXC_FROM_DIP
#endif
#if wxVERSION_NUMBER >= 3100
#define WXC_FROM_DIP(x) wxWindow::FromDIP(x, NULL)
#else
#define WXC_FROM_DIP(x) x
#endif


class MqttSetupDialogBase : public wxDialog
{
protected:
    wxBoxSizer* boxSizer3;
    wxFlexGridSizer* flexGridSizer5;
    wxStaticText* m_staticText13;
    wxTextCtrl* m_host;
    wxStaticText* m_staticText17;
    wxTextCtrl* m_ident;
    wxStaticText* m_staticText21;
    wxSpinCtrl* m_port;
    wxStaticText* m_staticText31;
    wxTextCtrl* m_username;
    wxStaticText* m_staticText35;
    wxTextCtrl* m_password;
    wxStaticText* m_staticText25;
    wxTextCtrl* m_filter;
    wxStaticText* m_staticText39;
    wxTextCtrl* m_commandSub;
    wxCheckBox* m_enable;
    wxStdDialogButtonSizer* m_stdBtnSizer7;
    wxButton* m_button9;
    wxButton* m_button11;

protected:
    virtual void OnOk(wxCommandEvent& event) { event.Skip(); }

public:
    wxStaticText* GetStaticText13() { return m_staticText13; }
    wxTextCtrl* GetHost() { return m_host; }
    wxStaticText* GetStaticText17() { return m_staticText17; }
    wxTextCtrl* GetIdent() { return m_ident; }
    wxStaticText* GetStaticText21() { return m_staticText21; }
    wxSpinCtrl* GetPort() { return m_port; }
    wxStaticText* GetStaticText31() { return m_staticText31; }
    wxTextCtrl* GetUsername() { return m_username; }
    wxStaticText* GetStaticText35() { return m_staticText35; }
    wxTextCtrl* GetPassword() { return m_password; }
    wxStaticText* GetStaticText25() { return m_staticText25; }
    wxTextCtrl* GetFilter() { return m_filter; }
    wxStaticText* GetStaticText39() { return m_staticText39; }
    wxTextCtrl* GetCommandSub() { return m_commandSub; }
    wxCheckBox* GetEnable() { return m_enable; }
    MqttSetupDialogBase(wxWindow* parent, wxWindowID id = wxID_ANY, const wxString& title = _("MQTT Configuration"), const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxSize(300,400), long style = wxDEFAULT_DIALOG_STYLE);
    virtual ~MqttSetupDialogBase();
};

#endif
