...
#ifndef __m_dataSourceModel_GUARD__H__
#define __m_dataSourceModel_GUARD__H__
 ...
 * To convert between a wxDataViewItem to a m_dataSourceModel_Item class, simply use:
 * @code
 * m_dataSourceModel_Item* itemData = reinterpret_cast<m_dataSourceModel_Item*>( item.GetID() );
 * if ( itemData ) {
 ...
 * @endcode
 */
class m_dataSourceModel_Item
{
protected:
    wxVector<wxVariant>        m_data;
    m_dataSourceModel_Item*           m_parent;
    wxVector<m_dataSourceModel_Item*> m_children;
    bool                       m_isContainer;
    wxClientData*              m_clientData;

public:
    m_dataSourceModel_Item()
        : m_parent(NULL)
        , m_isContainer(false)
        , m_clientData(NULL)
    {}
    virtual ~m_dataSourceModel_Item() {
        if ( m_clientData ) {
            delete m_clientData;
            m_clientData = NULL;
        }
        ...
        wxVector<m_dataSourceModel_Item*> tmpChildren = m_children;
     ...
     * @param child
     */
    void DeleteChild(m_dataSourceModel_Item* child) {
        wxVector<m_dataSourceModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
     ...
     * @param child
     */
    void RemoveChild(m_dataSourceModel_Item* child) {
        wxVector<m_dataSourceModel_Item*>::iterator iter = std::find(m_children.begin(), m_children.end(), child);
        if ( iter != m_children.end() ) {
            m_children.erase(iter);
        }
     ...
     */
    void AddChild(m_dataSourceModel_Item* child) {
    ...
    // Setters / Getters
    void SetChildren(const wxVector<m_dataSourceModel_Item*>& children) {
        this->m_children = children;
    }
    void SetData(const wxVector<wxVariant>& data) {
        this->m_data = data;
    }
    void SetParent(m_dataSourceModel_Item* parent) {
        this->m_parent = parent;
    }
    const wxVector<m_dataSourceModel_Item*>& GetChildren() const {
        return m_children;
    }
    wxVector<m_dataSourceModel_Item*>& GetChildren() {
        return m_children;
    }
    ...
    m_dataSourceModel_Item* GetParent() {
        return m_parent;
    }
...
class m_dataSourceModel : public wxDataViewModel
{
protected:
    wxVector<m_dataSourceModel_Item*> m_data;
    unsigned int m_colCount;

public:
    m_dataSourceModel();
    virtual ~m_dataSourceModel();
    ...
    void SetData(const wxVector<m_dataSourceModel_Item*> data) {
        this->m_data = data;
    }
    unsigned int GetColCount() const {
        return m_colCount;
    }
    const wxVector<m_dataSourceModel_Item*>& GetData() const {
        return m_data;
    }
    wxVector<m_dataSourceModel_Item*>& GetData() {
        return m_data;
    }
...
#endif // __m_dataSourceModel_GUARD__H__
