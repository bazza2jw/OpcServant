...
static class UnsignedStreamer : public MRL::VariantStreamer<unsigned int> {
    public:
...
void MRL::setJson(Json::Value &v, Variant &a) {
    ...
    switch (tn[0]) {
        ...
        case 'b':
            ...
            break;
        default:
            if (typeid(std::string).hash_code() == a.type().hash_code()) {
                v = boost::get<std::string>(a);
            }
...
std::string MRL::toString(const Variant &v) {
    ...
    switch (tn[0]) {
        ...
        default:
            if (typeid(std::string).hash_code() == v.type().hash_code()) {
                return boost::get<std::string>(v);
            }
