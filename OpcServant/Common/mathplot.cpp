...
bool mpWindow::SaveScreenshot(const wxString& filename, int /*type*/, wxSize imageSize, bool fit)
{
    ...
    return screenImage.SaveFile(filename, wxBITMAP_TYPE_PNG );
}

/*!
 * \brief mpWindow::ToImage
 * \param screenImage
 * \param imageSize
 * \param fit
 * \return true on success
 */
bool mpWindow::ToImage(wxImage &screenImage, wxSize imageSize, bool fit)
{
    int sizeX, sizeY;
    int bk_scrX, bk_scrY;
    if (imageSize == wxDefaultSize) {
    ...
    // Once drawing is complete, actually save screen shot
    screenImage = screenBuffer.ConvertToImage();
    return true;
}
