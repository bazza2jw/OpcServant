...
static void stuffed_tx_byte(struct min_context *self, uint8_t byte, bool crc)
{
    ...
    if (byte == HEADER_BYTE) {
        if (--self->tx_header_byte_countdown == 0) {
            min_tx_byte(self->context, STUFF_BYTE); // Stuff byte
...
static void on_wire_bytes(struct min_context *self, uint8_t id_control, uint8_t seq, uint8_t const *payload_base, uint16_t payload_offset, uint16_t payload_mask, uint8_t payload_len)
{
    ...
    min_tx_byte(self->context, EOF_BYTE);

    min_tx_finished(self->context);
}
...
static void send_ack(struct min_context *self)
{
    ...
    if (ON_WIRE_SIZE(0) <= min_tx_space(self->context)) {
    ...
    default:
        if (id_control & 0x80U) {
            ...
            if (seq == self->transport_fifo.rn) {
                ...
                min_application_handler(id_control & (uint8_t)0x3fU, payload, payload_len, self->context);
            } else {
...
void min_poll(struct min_context *self, uint8_t const *buf, uint8_t buf_len)
{
    uint8_t i;
    for (i = 0; i < buf_len; i++) {
        rx_byte(self, buf[i]);
    ...
    if ((window_size < TRANSPORT_MAX_WINDOW_SIZE) && (self->transport_fifo.n_frames > window_size)) {
        ...
        if (ON_WIRE_SIZE(frame->payload_len) <= min_tx_space(self->context)) {
        ...
        if ((window_size > 0) && remote_connected) {
            ...
            if (now - oldest_frame->last_sent_time_ms >= TRANSPORT_FRAME_RETRANSMIT_TIMEOUT_MS) {
                ...
                if (ON_WIRE_SIZE(oldest_frame->payload_len) <= min_tx_space(self->context)) {
...
void min_init_context_validate(struct min_context *self,  void *p_rx_frame_checksum, void * context)
#else
void min_init_context(struct min_context *self,  void * context)
#endif
{
#ifdef ASSERTION_CHECKING
    assert(self != 0);
    ...
    self->rx_header_bytes_seen = 0;
    self->rx_frame_state = SEARCHING_FOR_SOF;
    self->context = context; // context pointer - link to C++
...
{
    if ((ON_WIRE_SIZE(payload_len) <= min_tx_space(self->context))) {
