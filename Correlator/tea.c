...
multiplied by 2^32).
************************************************/
#define N_STRENGTH 32
void TEAencipher(unsigned long *const v,unsigned long *const w,
const unsigned long *const k)
{
	unsigned long       y=v[0],z=v[1],sum=0,delta=0x9E3779B9,
	a=k[0],b=k[1],c=k[2],d=k[3],n=N_STRENGTH;
	while(n-->0)
	{
		sum += delta;
		y += (z<<4)+a ^ z+sum ^ (z>>5)+b;
...
void TEAdecipher(unsigned long *const v,unsigned long *const w,
const unsigned long *const k)
{
