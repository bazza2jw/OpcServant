...
// initialise the DMA
// we dma from both ADC channels
// this gives us about 1MHz at 12 bits
//
#define DMA_COUNT 256
#define DMA_READ_SIZE 256
#define DMA_RAM_SIZE (DMA_COUNT * 2)
//
unsigned baseline = 0;
//
// initialise DMA to use ADC1
//
static int dmaCalls = 0;

void initDma0(void)
{
	DMA0CON=0x0002;					           // Continous Ping-Pong
	DMA0PAD=(volatile unsigned int) &ADC1BUF0; // read from ADC1
	DMA0CNT=DMA_COUNT;			
	DMA0STA= 0;				   // BUFFER A:		
	DMA0STB= DMA_RAM_SIZE;     // BUFFER B:
	DMA0CONbits.CHEN=0;
	DMA0REQ=13;
...
{
#if 0
	DMA1CON=0x0002;					           // Continous Ping-Pong
	DMA1PAD=(volatile unsigned int) &ADC2BUF0; // read from ADC2
	DMA1CNT=DMA_BASELINE;			
	DMA1STA= DMA_RAM_SIZE * 4;				   		// BUFFER A:		
	DMA1STB= DMA_RAM_SIZE * 4 + DMA_BASELINE * 2;   // BUFFER B:
	DMA1CONbits.CHEN=0;
	DMA1REQ=21;
	IFS0bits.DMA0IF = 0;			           //Clear the DMA interrupt flag bit
    IEC0bits.DMA0IE = 0;			           //Set the DMA interrupt enable bit
...
{
	DMA0CONbits.CHEN = 0;
    _DMA0IE  = 0;			           //Set the DMA interrupt enable bit
	_DMA0IF = 0;			           //Clear the DMA interrupt flag bit
};
...
//
// DMA interrupt handler - processes ADC data
//
//
//
...
{
#if 0
	static volatile unsigned *p1;
	static volatile unsigned *p2;
	static volatile unsigned v;
	static volatile unsigned m;
	...
	if(dmaCalls)
	{
		...
		for(i = 0; i < DMA_COUNT; i++,p1++,p2++)
		{
			v = *p1 ;
			if(v > baseline)
			{
				v -= baseline; // zero base the value
				for(j = 0; j < N_CHAN; j++)
				{
					C.Counts[j] += (v * (*p2));	
				};
			...
			//
			// shift the values along
			memmove(&C.ShiftReg[1], C.ShiftReg, sizeof(C.ShiftReg) - sizeof(unsigned));
			//
		};
	};
#endif
...
{
	int i;
	unsigned int *sp = Shift;
	unsigned long *cp = Counts;
	for(i = 0; i < 256; i++,cp++,sp++) *cp += *sp * 7;
};
#endif
...
void TestCorrelator(void)
{
	...
	//
	T1CON = 0;
	T1CONbits.TCKPS = 1; // divide by 8 - 5 ticks = 1us
	TMR1 = 0;
	PR1 = 0xFFFF;
    T1CONbits.TON = 1;
	IFS0bits.T1IF = 0;
